...
img_main = tkinter.PhotoImage(file=resource_path('image\\main.png'))
main = tkinter.Label(image=img_main)
main.place(x=0, y=0)

img_forward = tkinter.PhotoImage(file=resource_path('image\\fast-forward-1.png'))
forward = tkinter.Button(image=img_forward)
forward.place(x=412, y=270)

img_rewind = tkinter.PhotoImage(file=resource_path('image\\rewind-1.png'))
rewind = tkinter.Button(image=img_rewind)
rewind.place(x=172, y=270)

img_play = tkinter.PhotoImage(file=resource_path('image\\play-button-1.png'))
play = tkinter.Button(image=img_play)
play.place(x=252, y=270)

img_pause = tkinter.PhotoImage(file=resource_path('image\\pause-1.png'))
pause = tkinter.Button(image=img_pause)
pause.place(x=332, y=270)

img_plus = tkinter.PhotoImage(file=resource_path('image\\plus.png'))
plus = tkinter.Button(image=img_plus)
plus.place(x=560, y=105)

img_minus = tkinter.PhotoImage(file=resource_path('image\\minus.png'))
minus = tkinter.Button(image=img_minus)
minus.place(x=560, y=195)

img_folder = tkinter.PhotoImage(file=resource_path('image/folder-15.png'))
folder = tkinter.Button(image=img_folder, command=music_list)
folder.place(x=560, y=15)

img_disc = tkinter.PhotoImage(file=resource_path('image/compact-disc-1.png'))
